...
function managerHTML(mgr) {
    ...
    let template = fs.readFileSync(path.join(__dirname, './manager.html'), 'utf8');
    ...
    template = placeholderRegex(template, 'officeNumber', manager.getOfficeNumber());
...
function engineerHTML(eng) {
    ...
    let template = fs.readFileSync(path.join(__dirname, './engineer.html'), 'utf8');
...
function writeFile(data) {
    fs.writeFile(path.join(__dirname, '../team.html'), data, (err) => {
        if (err) throw err;
        console.log('Success!');
    })
}